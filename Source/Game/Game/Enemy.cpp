...
void Enemy::update(float deltaTime){
    ...
    if (player) {
      bonzai::vec2 direction{0,0 };
        ...
        float angle = bonzai::vec2::signedAngleBetween (forward,direction);


        angle = bonzai::math::sign(angle);
        if (playerSeen) {
            angle = bonzai::math::radToDeg(bonzai::vec2::angleBetween(forward, direction));
            playerSeen = (angle < 30);
...
void Enemy::onCollision(Actor* other){
    if (tag !=other->tag) {
        ...
        for (int i = 0; i < 100; i++) {
			...
			particle.velocity = bonzai::random::onUnitCircle() * bonzai::random::getReal(200.0f, 200.0f);
            particle.color = { 1,0,1 };
